...
- (NSString *)emailAddress {
    NSString *result = nil;
    NSXMLElement *email = [self elementForName:@"EMAIL"];
    if (email != nil) {
		// There is a email element. It should have a USERID
		NSXMLElement *emailAddress = [email elementForName:@"USERID"];
...
- (NSString *)phoneNumber {
    NSString *result = nil;
    NSXMLElement *tel = [self elementForName:@"TEL"];
    if (tel != nil) {
		// There is a tel element. It should have a NUMBER
		NSXMLElement *phoneNumber = [tel elementForName:@"NUMBER"];
...
- (void)setPhoneNumber:(NSString *)phoneNumber {
    ...
    //如果存在PhoneNumber节点，则先删除节点
    NSXMLElement *oldTel = [self elementForName:@"TEL"];
	if(oldTel){
        [self removeChildAtIndex:[[self children] indexOfObject:oldTel]];
    }
